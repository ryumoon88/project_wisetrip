@props(['activeManager', 'form' => null, 'formTabLabel' => null, 'managers', 'ownerRecord', 'pageClass'])
...
<div class="filament-resources-relation-managers-container space-y-2">
        ...
        <div class="flex justify-center">
            <x-filament::tabs>
                ...
                @foreach ($tabs as $tabKey => $manager)
                    ...
                    <button wire:click="$set('activeRelationManager', {{ filled($tabKey) ? "'{$tabKey}'" : 'null' }})"
                        @if ($activeManager === $tabKey) aria-selected @endif role="tab" type="button"
                        @class([
    ...
    @if (filled($activeManager) && isset($managers[$activeManager]))
        <div @if (count($managers) > 1) id="relationManager{{ ucfirst($activeManager) }}"
                role="tabpanel"
                tabindex="0" @endif
